 ...
 */
 ...
 export class TSMT$Quaternion
 {
  ...
  /**
   * Access the Quaternion values; return values in a four-element array
   */
   ...
   */
   public fromObject(q: {w: number, i: number, j: number, k: number}): void
   {
   ...
   */
   public toObject(): {w: number, i: number, j: number, k: number}
   {
  ...
  /**
   * Computes a 3-by-3 Euler rotation matrix from the current Quaternion
   */
   ...
   *
   * @param {TSMT$Quaterion} q Input quaternion for multiply
   */
   ...
   *
   * @param {TSMT$Quaterion} q Input quaternion for division
   */
   ...
   */
   public slerp( q: TSMT$Quaternion, _t: number ): TSMT$Quaternion
   {
     ...
     // cos of half-angle between quaternions
  ...
  /**
   * Normalized linear interpolation between the current and an input quaternion; returns normalized, interpolated quaternion at
   * the input t-parameter; note that NLERP does not preserve constant velocity, but is computationally simpler as well as commutative
   * and torque-minimal.
   *
   * @param {TSMT$Quaterion} q TSMT Quaternion
   *
